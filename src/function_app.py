...
@app.route(route="WeatherCollector", auth_level=func.AuthLevel.ANONYMOUS)
def WeatherCollector(req: func.HttpRequest) -> func.HttpResponse:
...
def DataProcessor(myblob: func.InputStream):
...
# @app.blob_trigger(arg_name="client", path="raw-data/{name}",
#                   connection="BlobStorageConnectionString")
# def DataProcessor(client: blob.BlobClient):
#     logging.info(
